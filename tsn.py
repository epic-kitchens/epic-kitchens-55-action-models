...
TSN Configurations:
    ...
    img_feature_dim:    {}
    dropout_ratio:      {}
        """.format(base_model, self.modality, self.num_segments, self.new_length, self.consensus_type,
                   self.img_feature_dim, self.dropout))
    ...
    def _prepare_tsn(self):
        if self.consensus_type.startswith("TRN") or not isinstance(self.num_class, (list, tuple)):
            ...
            assert(len(self.num_class) == 2), "We only support 2 tasks in multi task problems"
            setattr(self.base_model, self.base_model.last_layer_name, nn.Dropout(p=self.dropout))
    ...
    def logits(self, features: torch.Tensor) -> Union[torch.Tensor,
                                                      Tuple[torch.Tensor, ...]]:
        if isinstance(self.num_class, (list, tuple)) and not self.consensus_type.startswith("TRN"):
            logits_verb = self.fc_verb(features)
            if not self.before_softmax:
                logits_verb = self.softmax(logits_verb)
            if self.reshape:
                logits_verb = logits_verb.view((-1, self.num_segments) + logits_verb.size()[1:])
            ...
            if self.reshape:
                logits_noun = logits_noun.view((-1, self.num_segments) + logits_noun.size()[1:])
    ...
    def _construct_diff_model(self, base_model, keep_rgb=False):
        ...
        if not keep_rgb:
            ...
            new_kernel_size = kernel_size[:1] + (3 * self.new_length,) + kernel_size[2:]
            new_kernels = torch.cat((params[0].detach(), params[0].detach().mean(dim=1).expand(
...
def _vis_main(args):
    ...
    model = TSN(class_count, args.segment_count, args.modality, args.base_model,
                new_length, consensus_type=args.consensus)
    ...
    graph = make_dot(y, params=dict(list(model.named_parameters()) + [('x', x)]))  # type: graphviz.Graph
...
if __name__ == '__main__':
    ...
    parser.add_argument("arch_diagram", help="Path to save architecture diagram",
                        type=Path)
    parser.add_argument("--segment-count", default=3, type=int)
    parser.add_argument("--consensus", default="TRN",
                        choices=["avg", "max", "TRN", "TRNmultiscale"])
    parser.add_argument("--class-type", default='verb+noun',
    ...
    parser.add_argument("--modality", default='RGB',
                        choices=['RGB', 'Flow'])
    parser.add_argument("--base-model", default='BNInception',
                        choices=['BNInception', 'InceptionV3', 'resnet101',
                                 'vgg16', 'vgg19'])
    parser.add_argument("--format", default='pdf', help='Format to save graph in.These are 10 crop results.')
